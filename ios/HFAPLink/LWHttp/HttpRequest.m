...
//        NSString *md5Key = [self getAESKeyWithLocalIP:[LWNSHelper getIPAddress] withDeviceSSID:ssid];
        NSString *md5Key = [self getAESKeyWithLocalIP:[LWNSHelper getPhoneIPAddress] withDeviceSSID:ssid];
        NSString *ip = [LWNSHelper getPhoneIPAddress];
        ...
        LWLog(@"手机ip：%@  加密key：%@ 加密参数 netssID:%@ netPWD:%@ 加密后的密文: %@",[LWNSHelper getPhoneIPAddress],md5Key,netssid,pwd,result.base64);
...
+(void)connectModuleWithWiFiPwd:(NSString *)pwd
    ...
    // 生成aes 加密需要的密钥和向量
//    NSString *md5Key = [self getAESKeyWithLocalIP:[LWNSHelper getIPAddress] withDeviceSSID:ssid];
    NSString *md5Key = [self getAESKeyWithLocalIP:@"10.10.100.150" withDeviceSSID:@"hfsoftap_A6F6"];
    ...
    NSData *baseBodyData = [result.base64 dataUsingEncoding:NSUTF8StringEncoding];
    [self pritDataWith:baseBodyData];
    NSLog(@"%@ 转化后的%@",result.base64,[[NSString alloc] initWithData:baseBodyData encoding:NSUTF8StringEncoding]);
    
    
    //解密
    
    NSString *resultStr = @"sKkA7X7fqcwERqbp+9HiE4lTwGXZnoHp8FfDBxcSDMkWp50p1L5l4AxVXXsdJDyQeD288OkT439g5dXLm+xP9Q==";
    NSString *decryptStr = [resultStr aesBase64StringDecryptWithHexKey:md5Key hexIv:md5Key];
...
+(void)pritDataWith:(NSData *)data{
    //输出aes加密后的字符
    NSArray *bytes  = [self dataToByte:data];
    NSString *logString = @"";
